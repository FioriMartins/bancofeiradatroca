...
export default function SideBar() {
  ...
  const abrirMenu = () => {
    ...
    sidebardiv.current.style.width = '430px'

    timeoutsRef.current.push(setTimeout(() => {
      textButtons.current.forEach((p) => {
  ...
  const fecharMenu = () => {
    ...
    sidebardiv.current.style.width = '95px'
    textButtons.current.forEach((p) => {
  ...
  const handleMouseEnter = () => !clicado && abrirMenu()
  const handleMouseLeave = () => clicado && fecharMenu()
    ...
    <>
      ...
      >
        <div className='buttons'>
          <div id='icon' ref={(el) => (divButtons.current[1] = el)} onClick={() => { goToHome(), toggleClass(1) }}>
          ...
          <div id='icon' ref={(el) => (divButtons.current[2] = el)} onClick={() => { goToHome(), toggleClass(2) }}>
            ...
            <p ref={(el) => (textButtons.current[1] = el)}>Caixas</p>
          </div>
          <div id='icon' ref={(el) => (divButtons.current[3] = el)} onClick={() => { goToComandas(), toggleClass(3) }}>
            <svg width="70" height="70" viewBox="0 0 70 70" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className='button-config'>
          ...
          <div id='icon' ref={(el) => (divButtons.current[6] = el)} onClick={() => { goToConfig(), toggleClass(6) }}>
            <svg width="70" height="70" viewBox="0 0 70 70" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p ref={(el) => (textButtons.current[5] = el)}>Configuração</p>
          </div>
        </div>
      </div>
      <div className='blur'>
